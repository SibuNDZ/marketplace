          ...
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead>
              <tr style={{ textAlign: 'left', borderBottom: '1px solid var(--line)' }}>
                {['Order', 'Total', 'Items', 'Status', 'Actions'].map(h => (
            ...
            <tbody>
              {orders.map(o => (
                <tr key={o.id} style={{ borderBottom: '1px solid var(--line)' }}>
                  <td className="num" style={{ padding: '14px 12px', fontWeight: 700 }}>#{o.id}</td>
                  <td className="num" style={{ padding: '14px 12px' }}>R{Number(o.total).toFixed(2)}</td>
                  <td style={{ padding: '14px 12px', color: 'var(--ink-soft)', fontSize: 13 }}>{o.items.length}</td>
